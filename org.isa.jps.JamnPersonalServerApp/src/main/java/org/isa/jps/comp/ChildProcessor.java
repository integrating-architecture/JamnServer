...
import org.isa.jps.comp.ChildProcessManager.CPMJsonTool;
...
public class ChildProcessor {
     ...
     */
    protected void processMessage(String pJsonMsg) {

        if (pJsonMsg != null) {
            CPComData lMsgData = json.parse(pJsonMsg);

            if (lMsgData.hasInfo()) {
                LOG.info(() -> String.format("JPS Child Process Info [%s] [%s]", lMsgData.getInfo(), childId));
            }

            if (lMsgData.hasInfo(ChildProcessManager.HANDSHAKE_RESPONSE)) {
                LOG.info(() -> String.format("Child Process Handshake received [%s]", childId));
            } else if (lMsgData.isCmd()) {
                LOG.info(() -> String.format("Child Process Command received [%s] [%s]", lMsgData.getCommand(),
                        childId));

                if (lMsgData.isCmd(ChildProcessManager.CMD_CLOSE)) {
                    stop();
                }
            }
        }
    ...
    public synchronized void connect() {
        ...
        wsClient.sendText(json.from(new CPComData(childId).setInfo(ChildProcessManager.HANDSHAKE_REQUEST)), true);
    }
